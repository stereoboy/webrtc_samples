...
class PeerCombinedClient : public PeerClient,
    ...
    // std::unique_ptr<rtc::Thread> signaling_thread_;
public:
    PeerCombinedClient(): PeerClient("CombinedClient") {

    }

    virtual ~PeerCombinedClient() {
        if (data_channel_) {
            data_channel_->Close();
        ...
        logger_->info("PeerCombinedClient deleted");
    }
...
int main(int argc, char* argv[]) {
    ...
    auto logger = spdlog::stdout_color_mt("PeerCombined");
    absl::SetProgramUsageMessage(
      "Example usage: ./peer_combined_client --server=localhost --port=5000\n");
    absl::ParseCommandLine(argc, argv);

    logger->info("Starting PeerCombinedClient");
    // asio::ssl::context *ssl_ctx = new asio::ssl::context(asio::ssl::context::tls);

    auto client = rtc::make_ref_counted<PeerCombinedClient>();
